...
% Name of this effect
effect_name = 'Streatch';
write_audio = true;

%% User interface:

% Effect parameters with suggested initial value and typical range:
frame_size_ms = 10; % frame size in ms / 20ms / 1ms to 1000ms or higher
...
while ~isDone(audio_reader)
    ...
    % -
    Xm=upsample(Xm,Streatch);
    Xp=upsample(Xp,Streatch);
%     Xm1=Xm(1:(arraysize/2),:);
%     Xm2=Xm((arraysize/2+1):arraysize,:);
%     Xp1=Xp(1:(arraysize/2),:);
%     Xp2=Xp((arraysize/2+1):arraysize,:);
    
    halfoutU=(Streatch-1/2)*arraysize;
    halfoutL=(Streatch-1/4)*arraysize;    
    
%     Ym=[Xm1(1:size(Xm1,1)-halfoutL,:);Xm2(1:size(Xm2,1)-halfoutU,:)];
%     Yp=[Xp1(1:size(Xp1,1)-halfoutL,:);Xp2(1:size(Xp2,1)-halfoutU,:)];

    Ym=[Xm(1:halfoutL,:);Xm(halfoutU:arraysize,:)];
    Yp=[Xp(1:halfoutL,:);Xp(halfoutU:arraysize,:)];
