...
%  on the array length
loop_times = [29.7 37.1 41.1 43.7]; % loop time (s) / 30 <= loop_times <= 45 
...
delay_samples = loop_times * audio_reader.SampleRate;
g_comb = 10.^(-3*loop_times / RT60);

delay_ap = zeros(round(num_filters/2));
for i = 0:round(num_filters/2)-1
    delay_ap(i + 1) = (100e-3 / (3^i)) * audio_reader.SampleRate;
end
g_ap = 0.708;
